...
OpenAIが開発したコーディングエージェントです。公開自体は2025年4月にされています。
...
**/new**
会話履歴をリセットして新しい会話を作成することが出来ます。
タスクが完了したなど一区切りついた段階で使うことをお勧めします。
※コンソール上のテキストは残ったままです。

**ファイルメンション機能**
`@`でファイルをメンションし、コンテキストとしてエージェントに与えることが出来ます。
絶対パスを入力しなくてもファイル名を入力することであいまい検索がされるので便利です。

**推論のキャンセル**
間違えて送信した際にESCを押すことで中断することが出来ます。また、Escを2回押してEnterを押すことで以前のプロンプトの編集も可能です。
![推論キャンセル](/images/cancel.png)
...
**禁止コマンド**
毎回コマンドの承認をするのは手間なので、禁止コマンド設定が出来ないか調べましたが該当するような情報が見つかりませんでした。
Claude Codeでは可能みたいですが、今後のアップデートで対応するのでしょうか。
...
今回はサンプルコードをお見せしましたが、業務で1日利用してみたところ、十分活用できるレベルでした。UIの改善であったり、リファクタリングなどを効率的に行うことが出来ました。
...
- 実装面ではGit管理しておくことで差分が出せるので、マストだなと思いました。
