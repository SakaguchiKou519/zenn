...
![RUN命令を&&でまとめることでレイヤーが3層に削減された比較図](/images/docker_web/layer2.png)
...
`depends_on`の注意点としてコンテナの起動順を保証するだけでDBへの接続を保証しないことです。DBへの接続を保証したい場合は、`helthcheck`やアプリから接続失敗時にリトライする処理を入れることが有効です。
